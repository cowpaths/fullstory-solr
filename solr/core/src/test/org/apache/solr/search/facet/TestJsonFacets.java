...
public class TestJsonFacets extends SolrTestCaseHS {
  ...
  public static void beforeTests() throws Exception {
    ...
    log.info(
        "Setting FacetFieldProcessorByHashDV.MAXIMUM_STARTING_TABLE_SIZE = {}",
        FacetFieldProcessorByHashDV.MAXIMUM_STARTING_TABLE_SIZE);

    origCacheThreshold = TermFacetCache.DEFAULT_THRESHOLD;
    TermFacetCache.DEFAULT_THRESHOLD = /* nocommit: 1 + random().nextInt(50) */
        1; // stress test cache
    log.info("Setting TermFacetCache.DEFAULT_THRESHOLD = {}", TermFacetCache.DEFAULT_THRESHOLD);
  ...
  /**
   * When termFacetCache is (randomly) enabled, this test checks expected metrics after various
   * queries, but even if the cache is not enabled, this test sanity checks the basic behavior of
   * some simple term facets (and that use of the 'countCacheDf' request option doesn't cause any
   * weird failures if the cache does not exist)
   */
  public void testTrivialFacetWithCacheMetrics() throws Exception {
    ...
    // for all of these field types, basic behavior should be the same, and we should get cache hits
    // when
    ...
    for (String f :
      ...
      final boolean cacheShouldBeUsed =
          enableTermFacetCache
  ...
  public static void doStatsTemplated(Client client, ModifiableSolrParams p) throws Exception {
    ...
    */
    ...
    // nested query facets excludeTags for filters set in facet context, not main request (see
    // SOLR-9724)
    // this test is identical to "nested query facets on subset (with excludeTags)" (above), but
    // defines
