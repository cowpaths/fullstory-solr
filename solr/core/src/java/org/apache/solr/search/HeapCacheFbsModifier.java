  ...
  private void releaseLoop() throws InterruptedException {
    for (; ; ) {
      ...
      for (ByteBuffer bb : toRelease) {
        ...
        if (offheap) {
          // NOTE: for off-heap we only need to zero out the buffer if both:
          //   1. release doesn't call DONTNEED (if it does, it's zeroed in `madviseRelease()`
          //      below) AND
          //   2. bulk-faultin is disabled OR doesn't call MADV_POPULATE_WRITE (which would
          //      zero out the buffer upon acquire).
          if (MADV_RELEASE != MADV_DONTNEED
              && (!BULK_FAULT_IN || MADV_BULK_FAULTIN != MADV_POPULATE_WRITE)) {
